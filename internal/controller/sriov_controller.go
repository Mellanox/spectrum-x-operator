...
const (
	...
	sriovNetworkPoolType         = "SRIOVSriovNetworkPoolConfig"
	spectrumXSRIOVFinalizer      = "spectrumx.nvidia.com/sriovnetwork"
)
...
type SRIOVReconciler struct {
	...
	OVSNetworkNamespace string
}

//+kubebuilder:rbac:groups="",resources=configmaps/finalizers,verbs=update
//+kubebuilder:rbac:groups=sriovnetwork.openshift.io,resources=ovsnetworks,verbs=get;list;patch;delete;deletecollection
//+kubebuilder:rbac:groups=sriovnetwork.openshift.io,resources=sriovnetworknodepolicies,verbs=get;list;patch;delete;deletecollection
...
// reconcileOVSNetwork generates OVSNetowrks according to defined Rails
...
func (r *SRIOVReconciler) getDeviceNameByRail(deviceMapping []config.RailDeviceMapping, railName string) string {
	for _, mapping := range deviceMapping {
		if mapping.RailName == railName {
			return mapping.DevName
		}
	}
	return ""
}

// reconcileSRIOVNetworkPoolConfig generates SRIOVNetworkPoolConfig according to defined Rails
func (r *SRIOVReconciler) reconcileSRIOVNetworkPoolConfig(ctx context.Context, cfg *config.Config) error {
	logr := log.FromContext(ctx)
	for _, host := range cfg.Hosts {
		for _, rail := range host.Rails {
			devName := r.getDeviceNameByRail(cfg.RailDeviceMapping, rail.Name)
			network := r.generateSRIOVNetworkNodePolicy(host.HostID, rail.Name, devName, cfg.SpectrumXNetworks.MTU)
			logr.Info("Creating OVSNetwork", "name", network.Name)
			if err := r.Client.Patch(ctx, network, client.Apply, client.ForceOwnership, client.FieldOwner(spectrumXSRIOVControllerName)); err != nil {
				return fmt.Errorf("error while patching %s %s: %w", network.GetObjectKind().GroupVersionKind().String(), client.ObjectKeyFromObject(network), err)
			}
...
func (r *SRIOVReconciler) generateOVSNetwork(rail *config.Rail, mtu uint) *sriovnetworkv1.OVSNetwork {
	network := &sriovnetworkv1.OVSNetwork{
		ObjectMeta: metav1.ObjectMeta{
			Name:      rail.Name,
			Namespace: r.OVSNetworkNamespace,
			Labels: map[string]string{
				spectrumXSRIOVControllerName: "",
			},
		},
		Spec: sriovnetworkv1.OVSNetworkSpec{
			NetworkNamespace: r.OVSNetworkNamespace,
			ResourceName:     rail.Name,
			IPAM:             fmt.Sprintf(`{"type": "nv-ipam", "poolName": "%s"}`, rail.Name),
			MTU:              mtu,
		},
	}
...
func (r *SRIOVReconciler) generateSRIOVNetworkNodePolicy(host, railName, deviceName string, mtu uint) *sriovnetworkv1.SriovNetworkNodePolicy {
	nicSelector := &sriovnetworkv1.SriovNetworkNicSelector{
		PfNames: []string{deviceName},
	}
	nodeState := &sriovnetworkv1.SriovNetworkNodePolicy{
		ObjectMeta: metav1.ObjectMeta{
			Name:      railName,
			Namespace: r.OVSNetworkNamespace,
			Labels: map[string]string{
	...
	nodeState.ObjectMeta.ManagedFields = nil
	nodeState.SetGroupVersionKind(sriovnetworkv1.GroupVersion.WithKind(sriovNodePolicyType))
	return nodeState
}
...
func (r *SRIOVReconciler) deleteSRIOVNetworkNodePolicies(ctx context.Context) error {
	...
	if err := r.Client.DeleteAllOf(ctx, p, client.InNamespace(r.OVSNetworkNamespace), client.MatchingLabels{
		spectrumXSRIOVControllerName: "",
	}); err != nil {
	...
	return wait.PollUntilContextTimeout(ctx, interval, timeout, true, func(ctx context.Context) (bool, error) {
		...
		if err := r.Client.List(ctx, list, client.InNamespace(r.OVSNetworkNamespace), client.MatchingLabels{
			spectrumXSRIOVControllerName: "",
		}); err != nil {
