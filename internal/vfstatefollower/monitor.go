...
func (m *Monitor) tick(ctx context.Context) {
	...
	for i := range rpcList.Items {
		...
		enabled := rpc.Spec.VFStateFollow == nil || *rpc.Spec.VFStateFollow
		for _, rt := range rpc.Spec.RailTopology {
			...
			if enabled {
				...
				if err != nil {
					logr.Error(err, "failed to check PF carrier state", "topology", rt.Name)
					continue
				}
			}
			// When disabled, allDown is false: the restore branch runs and
			// brings up any representors that were previously set down.
			m.reconcileTopology(ctx, rt, allDown)
		}
	}
}

// reconcileTopology mirrors allDown to the VF representors whose VFs are inside pods.
// When allDown is false the restore branch runs, bringing any downed reps back up.
func (m *Monitor) reconcileTopology(ctx context.Context, rt v1alpha2.RailTopology, allDown bool) {
	logr := log.FromContext(ctx).WithName("vf-state-follower")
