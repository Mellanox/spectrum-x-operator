...
var _ = Describe("Monitor", func() {
	...
	Describe("reconcileTopology", func() {
		...
		Context("all PFs are NO-CARRIER (allDown=true)", func() {
			It("sets reps down when they are admin-UP", func() {
				...
				monitor.reconcileTopology(context.Background(), rt, true)
			})
		...
		Context("at least one PF is UP (allDown=false)", func() {
			It("restores reps to UP when they are admin-DOWN", func() {
				...
				monitor.reconcileTopology(context.Background(), rt, false)
			})
	...
	Describe("tick", func() {
		...
		It("restores downed reps when VFStateFollow is disabled (no stranded reps)", func() {
			// Simulate: monitor previously set rep down while feature was enabled.
			// Feature is now disabled — tick must still run the restore branch so
			// the rep comes back up without manual intervention.
			...
			monitor.FindActiveReps = func(pfName string) ([]string, error) {
				// eth99 (disabled RPC) has a downed rep; testPF0 (enabled RPC) has none
				if pfName == "eth99" {
					return []string{"eth99_0"}, nil
				}
				return []string{testRep0}, nil
			}

			// Enabled RPC: PF is up, rep is up — no action needed
			enabledRepLink := mocksnetlink.NewMockLink(ctrl)
			netlinkMock.EXPECT().LinkByName(testPF0).Return(mockLink, nil)
			netlinkMock.EXPECT().IsLinkNoCarrier(mockLink).Return(false)
			netlinkMock.EXPECT().LinkByName(testRep0).Return(enabledRepLink, nil)
			netlinkMock.EXPECT().IsLinkAdminStateUp(enabledRepLink).Return(true)

			// Disabled RPC: no PF carrier check; rep is down → must be restored UP
			disabledRepLink := mocksnetlink.NewMockLink(ctrl)
			netlinkMock.EXPECT().LinkByName("eth99_0").Return(disabledRepLink, nil)
			netlinkMock.EXPECT().IsLinkAdminStateUp(disabledRepLink).Return(false)
			netlinkMock.EXPECT().LinkSetUp(disabledRepLink).Return(nil)
